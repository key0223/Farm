...
{
    public WateringCan(ToolDataBase data)
    {
        Id = data.Id;
        Name = data.Name;
    ...
    {
        WateringCan tool  = clone as WateringCan;
    ...
    void Water(Vector3Int targetGrid)
    {
        GameLocation location = MapManager.Instance.CurrentLocation;

        HoeDirtFeature feature = location.GetRuntimeFeature(targetGrid.x, targetGrid.y) as HoeDirtFeature;
